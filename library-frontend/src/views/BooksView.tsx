...
import { Plus, Edit, Trash2, BookOpen, Search, Image as ImageIcon } from 'lucide-react';
import api from '../api';
...
const BooksView = ({ data, onRefresh }: { data: any, onRefresh: () => void }) => {
  ...
  const handleEdit = (book: Book) => {
    ...
    setFormData({
      ...
      authorIds: book.authors?.map((a: any) => a.id) || [data.authors[0]?.id || 1]
    });
    setIsModalOpen(true);
  };
  ...
  const handleDelete = async (id: number) => {
    if (confirm('Supprimer ce livre ?')) {
      try {
        ...
        alert('Erreur: ' + (error.response?.data?.message || 'Suppression impossible'));
      }
    }
  };
            ...
            >
              ...
              <div className="book-details">
                ...
                <div className="book-stock">
                  <span className={`stock-tag ${book.availableCopies > 0 ? 'in' : 'out'}`}>
                    {book.availableCopies} / {book.totalCopies} disponibles
                  </span>
                </div>

                <div className="book-actions">
                  <button className="btn-icon edit" onClick={() => handleEdit(book)} title="Modifier">
                    <Edit size={16} />
                  </button>
                  <button className="btn-icon delete" onClick={() => handleDelete(book.id)} title="Supprimer">
                    <Trash2 size={16} />
                  </button>
                </div>
              </div>
        ...
        <form onSubmit={handleSubmit} className="book-form">
          ...
          <div className="form-group">
            ...
            <div className="input-with-preview">
              <input 
                ...
                placeholder="https://..."
              />
              {formData.coverUrl && <img src={formData.coverUrl} className="preview-small" alt="Preview" />}
            </div>
          </div>
          ...
          <div className="form-row">
            ...
            <div className="form-group">
              ...
              <select 
                ...
                onChange={e => setFormData({...formData, authorIds: [parseInt(e.target.value)]})}
              >
          ...
          <div className="form-actions">
            ...
            <button type="submit" className="btn-primary" disabled={isSubmitting}>
              {editingBook ? 'Enregistrer les modifications' : 'Enregistrer le Livre'}
            </button>
          </div>
        </form>
      </Modal>
